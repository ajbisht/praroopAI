...
def build_scorecard(findings, doc_type="RFP", directory=None) -> dict[str, Any]:
    ...
    for r in rules:
        d = fw.setdefault(r["framework"], {"total": 0, "failed": 0,
                                           "mandatory_failed": 0, "pack": r["pack"]})
        d["total"] += 1
    for f in findings:
        d = fw.setdefault(f["framework"], {"total": 0, "failed": 0,
                                           "mandatory_failed": 0, "pack": f["pack"]})
        d["failed"] += 1
    ...
    total = sum(d["total"] for d in fw.values())
    failed = len(findings)
    overall = round(100 * (total - failed) / total) if total else 100
    mandatory_open = sum(1 for f in findings if f["severity"] == "mandatory")
    return {"rows": sorted(rows, key=lambda r: r["framework"]),
            "overall_percent": overall, "mandatory_open": mandatory_open,
            "can_finalize": mandatory_open == 0, "findings": findings}
