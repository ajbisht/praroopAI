...
def call_ollama(messages, **kw) -> str:
    ...
    r = httpx.post(url, json=body, timeout=settings.timeout); r.raise_for_status()
    d = r.json()
    if "eval_count" in d:
        log.debug("ollama tokens in=%s out=%s", d.get("prompt_eval_count"), d.get("eval_count"))
    return d["message"]["content"]


def _openai_like(base, key, messages, **kw) -> str:
    ...
    log.debug("POST %s model=%s", url, settings.model)
    r = httpx.post(url, json=body, headers=h, timeout=settings.timeout); r.raise_for_status()
    d = r.json()
    if "usage" in d:
        log.debug("tokens in=%s out=%s", d["usage"].get("prompt_tokens"), d["usage"].get("completion_tokens"))
    return d["choices"][0]["message"]["content"]


def call_openai(m, **kw):
    if not settings.openai_api_key: raise LLMError("OPENAI_API_KEY is empty in .env")
    return _openai_like(settings.openai_base_url, settings.openai_api_key, m, **kw)


def call_openrouter(m, **kw):
    if not settings.openrouter_api_key: raise LLMError("OPENROUTER_API_KEY is empty in .env")
...
def call_anthropic(messages, **kw) -> str:
    if not settings.anthropic_api_key: raise LLMError("ANTHROPIC_API_KEY is empty in .env")
    system, rest = _split_system(messages)
    url = f"{settings.anthropic_base_url.rstrip('/')}/v1/messages"
    h = {"x-api-key": settings.anthropic_api_key,
         "anthropic-version": settings.anthropic_version, "content-type": "application/json"}
    body = {"model": settings.model, "max_tokens": kw.get("max_tokens", settings.max_tokens),
            "temperature": kw.get("temperature", settings.temperature),
...
PROVIDERS = {"ollama": call_ollama, "openai": call_openai, "openrouter": call_openrouter,
             "anthropic": call_anthropic, "openai_compatible": call_compat}
